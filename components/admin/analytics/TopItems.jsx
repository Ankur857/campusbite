  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-2xl p-6 shadow-sm h-[400px] border border-gray-100 dark:border-zinc-800/85 hover:shadow-md transition-shadow duration-300">
      ...
      {sortedItems.length === 0 ? (
        <div className="text-center py-20 text-gray-500 dark:text-zinc-500 text-sm">
          No items sold yet.
        </div>
      ) : (
        <div className="space-y-6">
          {sortedItems.map((item) => (
            <div key={item.name}>
              <div className="flex justify-between text-sm mb-2 text-gray-700 dark:text-zinc-350 font-medium">
                <span className="truncate max-w-[170px]">{item.name}</span>
                <span className="text-orange-655 dark:text-orange-400 font-bold">{item.sold} sold</span>
              </div>

              <div className="w-full bg-slate-100 dark:bg-zinc-800 h-2 rounded-full">
                <div
